...
# -------------------------
# ANALOGÍAS (INTERACTIVO)
# -------------------------
elif pagina == "Analogías":
    ...
    palabras_input = st.text_input(
        ...
        "rey, hombre, mujer, parís, francia, italia"
    )

    palabras = [p.strip() for p in palabras_input.split(",")]

    cols = st.columns(len(palabras))

    for i, palabra in enumerate(palabras):
        if cols[i].button(palabra):
            st.session_state.formula.append(("word", palabra))

    col1, col2, col3 = st.columns(3)

    if col1.button("➕"):
        st.session_state.formula.append(("op", "+"))
    ...
    formula_str = ""
    for tipo, valor in st.session_state.formula:
        if tipo == "op":
            formula_str += f" <span style='color:red'>{valor}</span> "
        else:
            formula_str += f" {valor} "

    st.markdown(f"## {formula_str} = ?", unsafe_allow_html=True)
    ...
    if st.button("✨ Calcular"):
        ...
        sims = []
        for word, emb in zip(palabras, embeddings):
            sim = cosine_similarity([vector], [emb])[0][0]
            sims.append((word, sim))

        mejor = sorted(sims, key=lambda x: x[1], reverse=True)[0][0]

        st.markdown(
            f"<div class='result'>≈ {mejor}</div>",
            unsafe_allow_html=True
        )
